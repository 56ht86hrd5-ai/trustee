...
impl crate::Verifier for NvidiaDpuVerifier {
    async fn evaluate(
        ...
        // Inject the Trustee-issued challenge nonce (verified via alias-key signature
        // binding in step 5) as session_nonce claim — enables policy engines to
        // correlate attestation sessions.
        if let ReportData::Value(expected) = expected_report_data {
